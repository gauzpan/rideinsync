...
export function useActiveRide(userId: string | null, refreshKey?: string): ActiveRideState {
  ...
  useEffect(() => {
    ...
    return subscribeActiveRide(userId, (rideId) => setState({ rideId, loading: false }));
    // refreshKey in deps: a change forces cleanup (resubscribe) + a fresh
    // resolve, catching ride_members joins/leaves that realtime can't see.
  }, [userId, refreshKey]);
